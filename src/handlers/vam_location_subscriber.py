...
class VamLocationSubscriber(BaseSubscriberProcess):
    """Subscribes to VAM location topic and prints decoded payload."""

    def __init__(self, config_file: str, section: str = "VAM_LOCATION"):
        super().__init__("VamLocationSubscriber")
        self.config_file = config_file

        config = configparser.ConfigParser()
        config.read(self.config_file)
        self.topic = config.get(section, "topic", fallback="Hololens/VamLocation")
    ...
    def _subscriber_loop(self):
        ...
        self.logger.info("Starting VAM location subscription on topic: %s", self.topic)
        ...
        while not self._stop_event.is_set():
            ...
            try:
                ...
                self.logger.warning("Failed to decode VAM location packet: %s", exc)
                continue
            finally:
                self._emit_processing_ms((time.perf_counter() - t0) * 1000.0)

            self.logger.info("VamLocation latitude=%s longitude=%s", data.get("latitude"), data.get("longitude"))

    def _request_stop(self):
        self.logger.info("Requesting VAM location subscriber stop...")
        self._stop_event.set()
        self.buffer.put(None)

    def _subscriber_cleanup(self):
        self.logger.info("Cleaning up VAM location subscriber...")
        try:
